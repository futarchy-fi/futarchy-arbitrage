...
ABI_SELL = [
    {
        ...
        "inputs": [
            {"name": "amountPnkIn", "type": "uint256"},
            {"name": "minWethOut", "type": "uint256"},
            {"name": "minSdaiOut", "type": "uint256"},
        ],
        "outputs": [],
    }
]
...
def main():
    ...
    ap = argparse.ArgumentParser(description="Call FutarchyArbExecutorV5.sellPnkForSdai")
    ap.add_argument("--env", help=".env file path", default=None)
    ap.add_argument("--amount", help="PNK amount (ether units or 'ALL')", default="0.3")
    ap.add_argument("--min-weth", help="min WETH out (ether units)", default="0")
    ap.add_argument("--min-sdai", help="min sDAI out (ether units)", default="0")
    args = ap.parse_args()
    ...
    v5 = w3.eth.contract(address=w3.to_checksum_address(v5_addr), abi=ABI_SELL)
    ...
    print(f"Pre: PNK={w3.from_wei(pnk_before,'ether')} WETH={w3.from_wei(weth_before,'ether')} sDAI={w3.from_wei(sdai_before,'ether')}")

    if str(args.amount).upper() == 'ALL':
        amount_pnk_wei = int(pnk_before)
    else:
        amount_pnk_wei = w3.to_wei(Decimal(str(args.amount)), "ether")
        if amount_pnk_wei > pnk_before:
            raise SystemExit("Insufficient PNK balance on executor for requested amount")

    min_weth_wei = w3.to_wei(Decimal(str(args.min_weth)), "ether")
    min_sdai_wei = w3.to_wei(Decimal(str(args.min_sdai)), "ether")

    tx = v5.functions.sellPnkForSdai(int(amount_pnk_wei), int(min_weth_wei), int(min_sdai_wei)).build_transaction({
        "from": acct.address,
    ...
    print(f"Post: PNK={w3.from_wei(pnk_after,'ether')} WETH={w3.from_wei(weth_after,'ether')} sDAI={w3.from_wei(sdai_after,'ether')}")
