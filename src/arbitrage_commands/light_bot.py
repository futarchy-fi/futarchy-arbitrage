...
def buy_conditional_only(amount: float, broadcast: bool = False):
    ...
    # Step 1: Split sDAI
    split_tx = build_split_tx(acct.address, sdai_amount)
    ...
    if broadcast:
        ...
        # Get actual amounts from receipts
        receipts = [w3.eth.wait_for_transaction_receipt(h) for h in tx_hashes]
        
        # Parse YES swap
        yes_result = parse_broadcasted_swapr_results(receipts[1])
        yes_amount_out = Decimal(yes_result["amountOut"]) / 10**18
        
        # Parse NO swap
        no_result = parse_broadcasted_swapr_results(receipts[2])
        no_amount_out = Decimal(no_result["amountOut"]) / 10**18
        
        # Merge what we can
        merge_amount = min(yes_amount_out, no_amount_out)
        if merge_amount > 0:
            merge_tx = build_merge_tx(acct.address, int(merge_amount * 10**18))
        ...
        # Simulate only
        sim_id = client.simulate_bundle(bundle)
        time.sleep(0.5)
        sims = client.get_simulation_bundle(sim_id)
